...
class GenerateCode(NodeVisitor):
    ...
    def __init__(self, cfg):
        ...
        # self.array_ref = Fila()
        self.binary_bool = {"==": "eq", "!=": "ne", "<": "lt", ">": "gt", "<=": "le", ">=": "ge", "&&": "and", "||": "or"}
        self.binary_ops = {"+": "add", "-": "sub", "*": "mul", "/": "div", "%": "mod"}
        self.unary_ops = {"-": "sub", "+": "", "--": "sub", "++": "add", "p--": "sub", "p++": "add", "!": "not",
                          "*": "", "&": ""}
        self.assign_op = {"+=": "add", "-=": "sub", "*=": "mul", "/=": "div", "%=": "mod"}
    ...
    def visit_Program(self, node):
        ...
        for _decl in node.gdecls:
            if isinstance(_decl, FuncDef):
                ...
                # opt.optimization()
                # _decl.reset()
                _llvm = LLVMFunctionVisitor(_decl.blocks)
                _llvm.gen_llvm()
                if (self.cfg):
    ...
    def visit_FuncDef(self, node):
        ...
        inst_def = [f'define_{node.spec.names}', '@' + name, []]
        ...
        # self.code.append(inst2)
        self.blocks.append(self.block_current)  # Guardamos tal bloco
    ...
    def visit_ArrayRef(self, node):
        ...
        if (isinstance(node.name, ID)):
            ...
            inst2 = [f'load_{type}', var1, tgt2]
            inst3 = [f'mul_int', tgt1, tgt2, tgt3]
    ...
    def visit_UnaryOp(self, node):
        ...
        if (not (node.op == '&')):
            ...
            node.gen_location = escopo.table.lookup(name)
            ...
            for nod in decls:
                for no in nod:
                    ...
                    if (no.arraydecl and no.const):
                        ...
                        inst = [f'store_{no.vardecl.type.names}', aux2, aux1]
                        self.code.append(inst)
                        self.block_current.append(self.increase_count(), inst)
                    elif (no.binop):
                        self.visit(no.binop)
    ...
    def clear_None(self):
        code = []
        for i in range(len(self.code_opt)):
            if (self.code_opt[i][0]):    code.append(self.code_opt[i])
        self.code_opt = code
